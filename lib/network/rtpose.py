...
from .rtpose_mobilenetV33 import mobilenetv3
...
def get_model(trunk='vgg19'):
    ...
    if trunk == 'vgg19':
        ...
        # output : 128, 46, 46
        models['block0'] = mobilenetv3(pretrained=True)
...
def use_mobilenet(model):
    ...
    weights_load = {}
    # weight+bias,weight+bias.....(repeat 10 times)
    for i in range(len(mobilenet_keys)):
        if('model0' not in list(model.state_dict().keys())[i]):
            break
        else:
