...
def check_rotation(path_video_file):
    ...
    # we are looking for
    rotateCode = None
    if meta_dict['streams'][0]['tags'].get('rotate') is not None:
...
times = []
torchsummary.summary(model, (3,368,368))
if __name__ == "__main__":
    video_path = "/src/YUNAKIM_Trim.mp4"#input("Enter video path")
    ...
    vid_out = cv2.VideoWriter('output.avi', fourcc, 30.0, shape_tuple)
    ###
    ...
    for oriImg in oriImg_list:
        ...
        vid_out.write(out)
    print(" min : {}ms at index {} max : {}ms at index {} average : {}ms".format(min(times)*1000,np.argmin(times),max(times)*1000,np.argmax(times),sum(times)*1000/len(times)))
